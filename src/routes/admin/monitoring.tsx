...
import {
  Loader2, ShieldCheck, Activity, Users, Car, AlertTriangle,
  TrendingUp, ChevronLeft, Eye, Ban, Flag, Headphones, Database,
} from "lucide-react";
...
function LiveMonitoring() {
  ...
  const [tab, setTab] = useState<"live" | "reports" | "blocks" | "support">("live");
  ...
  const loadData = useCallback(async () => {
    ...
    setStats({
      ...
      activeRequests: activeReq.count ?? 0,
    });
    setReports(pendingRep.data ?? []);
    setBlocks(blk.data ?? []);
    setTickets(tck.data ?? []);
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { if (authorized) loadData(); }, 15000);
    ...
    return (
      <div className="min-h-screen grid place-items-center px-5">
        <div className="text-center">
          ...
          <Link to="/admin" className="inline-block mt-4 text-primary text-sm font-bold">العودة للوحة الإدارة</Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen px-5 pt-10 pb-20 max-w-2xl mx-auto">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h1 className="text-2xl font-black">المراقبة المباشرة</h1>
      </div>
      ...
      {tab === "live" && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <LiveStat icon={Users} label="إجمالي المستخدمين" value={stats.totalUsers} />
            <LiveStat icon={Car} label="سائقون نشطون" value={stats.activeDrivers} accent="text-emerald-600" />
            <LiveStat icon={TrendingUp} label="فنيون نشطون" value={stats.activeWorkers} accent="text-amber-600" />
            <LiveStat icon={Activity} label="طلبات نشطة" value={stats.activeRequests} accent="text-blue-600" />
          </div>
          <div className="glass rounded-2xl p-3 text-center text-xs text-muted-foreground">
            <Database className="h-4 w-4 inline ml-1" />
            يتم تحديث البيانات كل 15 ثانية
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {reports.length === 0 ? (
            <div className="glass rounded-2xl p-8 text-center">
              ...
              <div className="text-sm text-muted-foreground">لا توجد بلاغات معلقة</div>
            </div>
          ) : (
            reports.map((r) => (
              <div key={r.id} className="glass rounded-2xl p-3 text-sm">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString("ar-IQ")}</span>
                </div>
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    onClick={async () => {
                      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                      await (supabaseAdmin as any).from("user_reports").update({ status: "resolved", resolved_at: new Date().toISOString(), resolved_by: session.user.id }).eq("id", r.id);
                      toast.success("تم حل البلاغ");
                      loadData();
                    }}
                    className="text-xs px-3 py-1 rounded-lg bg-emerald-100 text-emerald-700 font-bold"
                  >
                    حل
                  </button>
                  <button
                    onClick={async () => {
                      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                      if (r.reported_id) {
                        await (supabaseAdmin as any).from("user_blocks").upsert({ user_id: r.reported_id, blocked_by: session.user.id, reason: r.reason }, { onConflict: "user_id" });
                        toast.success("تم حظر المستخدم");
                      }
                      await (supabaseAdmin as any).from("user_reports").update({ status: "resolved", resolved_at: new Date().toISOString(), resolved_by: session.user.id }).eq("id", r.id);
                      loadData();
                    }}
                    className="text-xs px-3 py-1 rounded-lg bg-red-100 text-red-700 font-bold"
                  >
                    حظر المستخدم
                  </button>
                </div>
        ...
        <div className="space-y-2">
          {blocks.length === 0 ? (
            <div className="glass rounded-2xl p-8 text-center">
              ...
              <div className="text-sm text-muted-foreground">لا يوجد مستخدمون محظورون</div>
            </div>
          ) : (
            blocks.map((b) => (
              <div key={b.id} className="glass rounded-2xl p-3 text-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-bold">{(b as any).profiles?.full_name ?? b.user_id.slice(0, 8)}</div>
                    {b.reason && <div className="text-xs text-muted-foreground">{b.reason}</div>}
                  </div>
                  <button
                    onClick={async () => {
                      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                      await (supabaseAdmin as any).from("user_blocks").delete().eq("user_id", b.user_id);
        ...
        <div className="space-y-2">
          {tickets.length === 0 ? (
            <div className="glass rounded-2xl p-8 text-center">
              ...
              <div className="text-sm text-muted-foreground">لا توجد تذاكر دعم مفتوحة</div>
            </div>
          ) : (
            tickets.map((t) => (
              <Link key={t.id} to="/support" className="block glass rounded-2xl p-3 text-sm">
                <div className="flex items-center justify-between">
...
function LiveStat({ icon: Icon, label, value, accent }: any) {
  return (
