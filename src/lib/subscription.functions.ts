...
export const checkSubscriptionAllowed = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await (supabase as any).rpc("check_request_allowed", {
      _user_id: userId,
    });
...
export const getUserSubscription = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { supabase, userId } = context;
    const { data, error } = await (supabase as any).rpc("get_user_subscription", {
      _user_id: userId,
    });
    if (error || !data || data.length === 0) {
      return null;
...
export const incrementUserRequestCount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const { supabase, userId } = context;
    await (supabase as any).rpc("increment_request_count", { _user_id: userId });
    return { ok: true };
  });
